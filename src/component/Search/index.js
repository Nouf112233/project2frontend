...
function Search() {
    ...
    const getSearched = async () => {
          ...
          const search = await axios.get('http://localhost:5000/product/search',{name:name});
          search.data.forEach(element => {
              console.log("element",element)
            if(element.kind.toLowerCase().includes(name)||element.name.toLowerCase().includes(name)){
